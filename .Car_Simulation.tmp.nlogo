...
to setup
  ...
  ;; Environment
  ask patches [ ifelse pycor < 3 and pycor > -3 [ set pcolor gray ] [ set pcolor green ] ]
...
to go
  ask turtles [
    let next-car one-of turtles-on patch-ahead ; defineert next-car als de auto die (eventueel) op de voorliggende patch rijd.
